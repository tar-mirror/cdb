...
/* adapted for use in dcache by Uwe Ohse, uwe@ohse.de.
 * no changes to copyright or usage non-restriction.
 */
...
/* 
 * i took this from lrzsz and changed to function names. -- uwe
 */
...
static dcache_uint32 crc_32_tab[] = { /* CRC polynomial 0xedb88320 */
...
void 
dcache_hash_init(dcache_uint32 *crc)
{
	*crc = 0xFFFFFFFFUL;
}

void 
dcache_hash_update(dcache_uint32 *pcrc, const char *buf, dcache_reclen laenge)
{
	dcache_uint32 crc=*pcrc;
	while (laenge--)
...
void 
dcache_hash_finish(dcache_uint32 *crc)
{
	*crc= (~(*crc)) & 0xffffffff; /* in case dcache_uint32 is longer */
}

void 
dcache_hash(dcache_uint32 *pcrc, const char *puffer,dcache_reclen laenge)
{
	dcache_hash_init(pcrc);
	dcache_hash_update(pcrc,puffer,laenge);
	dcache_hash_finish(pcrc);
}
