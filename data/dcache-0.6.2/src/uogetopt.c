...
static uogetopt2 hack_help=
  UOGO_O(0,"help",   uogo_flag,UOGO_NOARG,&dummy,0,
       /* 12345678901234567890123456789012345678901234567890" */
#ifdef UOGO_SMALL
	 "Show the list of options.",
#else
	 "Show the a list of options or the long help on one.",
#endif
...
#define OUT(s) do {env->out(env,s);} while(0)
...
#else
  if (argv[1] && str_equal(argv[1],minus_help)) {
    env->fd=1;
    uogetopt_printhelp(env,PRINTHELP_MODE_SHORT);
    env->fd=2;
    goto exithelpversion;
  }
#endif
  ...
  for (i=1;i<*argc;i++) {
	...
	*para++=0;
	...
	if (o->L && str_equal(o->L,argv[i]+2)) 
	  break;
      if (j==ocount) {
	O4(env->program,": illegal option -- ",argv[i],"\n");
	goto exiterr;
      }
      if (ioff==1) {
	...
	O4(env->program,": option doesn't allow an argument -- ",argv[i],"\n");
	goto exiterr;
      }
      if ((o->flags & UOGO_NOARG)==0 && !para) {
	O4(env->program,": option requires an argument -- ",argv[i]+2,"\n");
	goto exiterr;
      }
	...
	for (k=0;k<ocount;k++,o++)
	  if (o->S && o->S==c) 
	    break;
	if (k==ocount) {
	  O4(env->program,": illegal option -- ",optstr,"\n");
	  goto exiterr;
	}
	...
	if (!p && !was_flag) {
	  O4(env->program,": option requires an argument -- ",
		  optstr,"\n");
	  goto exiterr;
	}
  ...
  *argc=newargc;
  argv[newargc]=0;
  checkrest:
#define COMMONSTR " arguments. Use the --help option for more information.\n"
    if (env->minargs && newargc < env->minargs) {
	  O2(env->program,": need more" COMMONSTR);
	  goto finish_argcount;
	}
    if (env->maxargs && newargc > env->maxargs) {
	  O2(env->program,": too many" COMMONSTR);
   finish_argcount:
#ifndef UOGO_SMALL
