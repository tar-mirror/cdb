...
gcctest() {
  if $1 --version /dev/null 2>&1 |head -n 1 |egrep '^(egcs|gcc)' >/dev/null ; then
    echo 1
  else
    echo 0
  fi
...
#include "gen_fmt.h"
#include "fmt.h"
FMT_UFUNC(ulong,unsigned long,10,"0123456789")
int main(void)
{
  char b[FMT_ULONG];
  off_t x=0;
  int y;
  y=fmt_ulong(b,8*sizeof(x));
  b[y++]='\n';
  b[y++]=0;
  if (y != write(1,b,y)) return 1;
  return 0;
}
EOF
